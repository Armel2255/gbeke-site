...
const MatinSection: React.FC<{ images: typeof DEFAULT_IMAGES }> = ({ images }) => {
  ...
  const handleOrder = (product: any) => {
    saveOrder(product);
    let msg = `Bonjour ! Je veux commander : ${product.name} (${product.price} F) pour mon petit-déjeuner.`;
    if (product.price >= DELIVERY_FREE_THRESHOLD) {
      msg += ` Est-ce que la livraison est gratuite pour ${DELIVERY_LOCATIONS} ?`;
    }
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20">
      <div className="flex items-center justify-between px-2">
        ...
        <span className="text-[9px] bg-brand-primary text-white px-3 py-1.5 rounded-full font-black uppercase tracking-widest">En direct du four</span>
      </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start">
                ...
                {product.price >= DELIVERY_FREE_THRESHOLD && (
                   <i className="fa-solid fa-truck-fast text-brand-secondary text-xs"></i>
                )}
              </div>
